...
Follow the visual-explainer skill workflow. Read the skill's `templates/slide-deck.html` and `references/slide-patterns.md` before generating. Also read the skill's `references/css-patterns.md` for shared patterns (Mermaid zoom controls, depth tiers, overflow protection) and `references/libraries.md` for Mermaid theming, Chart.js, and font pairings.
